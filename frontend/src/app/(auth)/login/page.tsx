...
import { Sparkles, Github, Globe, Mail, Lock, ArrowRight } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      setError('Could not connect to Google auth service.');
      setLoadingGoogle(false);
    }
  };
  ...
  const handleGitHubLogin = async () => {
    ...
    try {
      ...
      setError('Could not connect to GitHub auth service.');
      setLoadingGitHub(false);
    }
  };
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.access_token && data.user) {
        ...
        setError('Received an invalid session payload from authentication.');
        setLoadingEmail(false);
      }
    } catch (err: any) {
      console.error('Failed email/password login:', err);
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 flex items-center justify-center font-sans px-4 overflow-hidden relative transition-colors duration-300">
      ...
      <div className="w-full max-w-md bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-slate-850 p-8 rounded-3xl shadow-xl backdrop-blur-md z-10 flex flex-col space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl font-extrabold text-slate-800 dark:text-white tracking-tight">Welcome to CoFoundr</h1>
          <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Log in to manage startup analytics and advisory threads</p>
        </div>
        ...
        <div className="flex items-center justify-center gap-3">
          ...
          <span className="text-[10px] font-bold text-slate-450 dark:text-slate-500 uppercase tracking-widest">Or Account Credentials</span>
          <div className="h-px bg-slate-200 dark:bg-slate-850 flex-1"></div>
        </div>
        ...
        <form onSubmit={handleEmailLogin} className="flex flex-col space-y-4">
          ...
          <button
            type="submit"
            disabled={!email || !password || loadingGoogle || loadingGitHub || loadingEmail}
